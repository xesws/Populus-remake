...
import { BLUE, BuildingKind, FxBolt, houseHp, inMap, isCampKind, Order, RED, snapYaw, Tool, TrainKind, WORLD } from "./types";
...
export class Game {
  ...
  maybeHoldSwamp(): void {
    ...
    if (this.shotBeat === 1 && !this.paused) {
      ...
      } else if (outsider && stuck.some((u) => u.swampT > 0.15)) {
        const u = stuck[0]!;
        u.selected = true;
        this.view.jump((u.x * 0.7 + outsider.x * 0.3), (u.z * 0.7 + outsider.z * 0.3));
        this.view.dist = 6.4;
        this.paused = true;
        this.shotBeat = 2;
        this.sim.toast("拍2 · 人被堵住");
        this.hud.toastT = 30;
        return;
      }
    }
  ...
  maybeHoldLightning(): void {
    if (typeof location === "undefined" || !location.search.includes("shot=lightning")) return;
    if (this.ended) return;
    const hut = this.sim.buildings.find((b) => b.id === this.shotVictimId);
    let walker = this.sim.units.find((u) => u.id === this.shotWalkerId);
    const bolt = this.bolts.find((b) => b.life > 0) ?? this.sim.fxBolts.find((b) => b.life > 0);
    if (this.shotBeat === 0 && bolt && !this.paused) {
      this.paused = true;
      this.shotBeat = 1;
      this.view.hover(0, 0, false);
      this.view.jump(bolt.x1, bolt.z1);
      this.view.look.y = this.world.heightAt(bolt.x1, bolt.z1) + 2.6;
      this.view.dist = 9.4;
  ...
  frame(now: number): void {
    ...
    if (this.placeKind && cell && inMap(cell.x, cell.z)) {
      ...
      this.view.hideGhost();
      if (this.paused) {
        if (this.fightHoverMode === "fight") this.view.hover(this.fightHoverX, this.fightHoverZ, true, "fight");
    ...
    if (this.running && !this.paused && !this.ended) {
      ...
      this.sim.tick(dt);
      const shot = typeof location !== "undefined" && (location.search.includes("shot=1") || location.search.includes("shot=swamp") || location.search.includes("shot=volcano") || location.search.includes("shot=quake") || location.search.includes("shot=skel") || location.search.includes("shot=tornado") || location.search.includes("shot=lightning") || location.search.includes("shot=blast") || location.search.includes("shot=house") || location.search.includes("shot=path") || location.search.includes("shot=live") || location.search.includes("shot=fight"));
      if (!shot) this.ai.update(this.sim, dt);
